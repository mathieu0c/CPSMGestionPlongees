...
bool save(const QJsonDocument& doc,const QString& fileName)
{
    ...
    if(!f.open(QIODevice::WriteOnly | QIODevice::Truncate)){
        if constexpr(throwErrorOnFileFail)
            throw std::runtime_error{"bool save(const QJsonDocument& doc,const QString& fileName) : Cannot open file (w) "+fileName.toStdString()};
        else
            return false;
    }
...
std::optional<QJsonDocument> read(const QString& fileName)
{
    ...
    if(!f.open(QIODevice::ReadOnly)){
        if constexpr(throwErrorOnFileFail)
            throw std::runtime_error{"std::optional<QJsonDocument> read(const QString& fileName) : Cannot open file (r) "+fileName.toStdString()};
        else
        return {};
    }
